...
<?php 
   ...
   if(isset($_POST['donate_blood'])){
     try{
       if(empty($_POST['blood_group'])||empty($_POST['d_address'])){
         throw new Exception('Please give the required informations!');
       }
       $post_time = time();
        $query = $db->prepare("INSERT INTO timeline(member_id,db_group,d_address,`timestamp`) VALUES(?,?,?,?)");
        $result = $query->execute(array($_SESSION['id'],$_POST['blood_group'],$_POST['d_address'],$post_time));
   ...
   ?>
    ...
    <div id="sideNavbar">
      <div class="navbar">
        <ul class="navbar-nav">
          ...
          <li class="nav-item active"><a href="donate.php" class="nav-link">
    ...
    <div class="header">
      ...
      <div id="search-blood">
        <h4>Create a post about blood donation</h4>
        ...
        ?>
          <form action="" method="POST">
            <label for="">You are donating</label>
            ...
            <br>
            <input type="submit" name="donate_blood" value="Create Post" class="btn btn-md btn-danger">
          </form>
        </div>
      </div>
    </div>
